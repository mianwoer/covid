...
def zgdt():  # 中国现有确诊地图
    df = pd.read_csv('covid_ksh_demo/csv/国内疫情数据.csv')
...
def gfx(t):
    ...
    with open(f"covid_ksh_demo/data/{t}.json", "r", encoding="utf-8") as f:
        j = json.load(f)
    if t == '高风险':
        x = 'orangered'
        y = 1.5
    ...
    c = (
        Tree()
            .add("", [j], collapse_interval=20, is_roam=True, layout="radial", initial_tree_depth=y,
                 label_opts=opts.LabelOpts(color='#ffffff', font_size=9))
            .set_global_opts(
            title_opts=opts.TitleOpts(title=f"{t}地区", title_textstyle_opts=opts.TextStyleOpts(color=x, font_size=16), ))
    )
    return c.dump_options_with_quotes()
